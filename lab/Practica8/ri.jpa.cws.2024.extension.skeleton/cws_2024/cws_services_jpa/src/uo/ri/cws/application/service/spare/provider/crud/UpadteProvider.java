...
public class UpadteProvider implements Command<Void> {
    ...
    public Void execute() throws BusinessException {
        ...
        List<Provider> providers = re.findByNameMailPhone(dto.name, dto.email,
            dto.phone);

        BusinessChecks.isTrue(providers.size() == 0,
            "existe un mecanico con esos datos");

        Provider pro = provider.get();
        BusinessChecks.hasVersion(dto.version, pro.getVersion(),
